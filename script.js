...
const renderCalendar = () => {
 ...
 const prevLastDay = new Date(
  ...
  /*for (let i = 1; i <= lastDay; i++) {
    for (let h in USHolidays){
        let d="";
        d=USHolidays[h].split(".")
        console.log(d[0], date.getDate(), d[1], date.getMonth())
        if ((d[0])===date.getDate().toString() && d[1]===(date.getMonth()+1).toString()){
...
var parties = {};
    ...
    for (let i in parties.parties) {
       party.innerHTML +=
      `<a class="dropdown-item" href="#" style="font-size: 1.5rem"> ${parties.parties[i].name} </a>`
          }
      //* de pus aici in # linkul pentru fiecare party in parte
      //* de modificat culori 
  }
...
renderCalendar();
>>>>>>> 846d8853f9686ab98557aae4b1fea24f11a70b86
